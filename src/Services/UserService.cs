...
namespace b2c_ms_graph
{
    class UserService
    {
        public static async Task ListApplications(GraphServiceClient graphClient)
        {
            Console.WriteLine("Getting list of applications...");

            var servicePrincipals = await graphClient.ServicePrincipals
                    .Request()
                    .GetAsync();

            foreach (var user in servicePrincipals.CurrentPage)
            {
                Console.WriteLine(JsonConvert.SerializeObject(user));
            }

            var result = await graphClient.Applications.Request()
                    .Select(e => new
                    {
                        e.AppId,
                        e.DisplayName,
                        KeyCredential = e.KeyCredentials
                    }).GetAsync();
